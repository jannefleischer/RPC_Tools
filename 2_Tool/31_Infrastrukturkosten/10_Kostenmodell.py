...
#projektname = 'LKH_Bultweg_dicht' # haendische Ueberbrueckung bei manueller Scriptnutzung
#entwaesserungssystem = "Mischsystem" #"Trennsystem", "Mischsystem","Oberflaechenversickerung"
...
workspace_projekt_kosten = os.path.join(base_path,'3_Projekte',projektname,'FGDB_31_Kosten_'+projektname+'.gdb')
workspace_tool_kosten = os.path.join(base_path,"2_Tool","31_Infrastrukturkosten","FGDB_31_Kosten_Tool.gdb")

flaechenbilanz_planung = os.path.join(workspace_projekt_definition,'Flaechenbilanz_Planung_Prozent')
flaechenbilanz_planung_gruppiert = os.path.join(workspace_projekt_definition,'Flaechenbilanz_Planung_gruppiert')
...
eingangstabellen = [
    (workspace_projekt_definition,'Flaechenbilanz_Planung_Prozent')
]

ausgabetabelle = (workspace_projekt_definition,'Flaechenbilanz_Planung_gruppiert')

sql = """SELECT Flaechenbilanz_Planung_Prozent.Teilflaeche_Plangebiet,Flaechenbilanz_Planung_Prozent.Startjahr, Flaechenbilanz_Planung_Prozent.Flaechennutzung_S1, Flaechenbilanz_Planung_Prozent.Flaechennutzung_S2, Sum(Flaechenbilanz_Planung_Prozent.Flaeche_ha) AS Flaeche_ha
INTO Flaechenbilanz_Planung_gruppiert
FROM Flaechenbilanz_Planung_Prozent
GROUP BY Flaechenbilanz_Planung_Prozent.Teilflaeche_Plangebiet, Flaechenbilanz_Planung_Prozent.Startjahr, Flaechenbilanz_Planung_Prozent.Flaechennutzung_S1, Flaechenbilanz_Planung_Prozent.Flaechennutzung_S2;
"""
...
##                message = element_name, rel_menge
...
sql = """SELECT MEG_Mengenermittlung.Teilflaeche_Plangebiet, T01DEF_Untersuchungsjahre.Jahr, MEG_Mengenermittlung.Kostenbereich, T03MEG_Element_und_Preisliste.Element AS Element, [Menge]*[Herstellungskosten_einmalig]*[Projektspezifischer_Sonderkostenfaktor]*[Faktor_BKI] AS Herstellungskosten INTO T04KOM_01_Herstellungskosten
FROM (MEG_Mengenermittlung INNER JOIN T01DEF_Untersuchungsjahre ON MEG_Mengenermittlung.Startjahr = T01DEF_Untersuchungsjahre.Jahr) INNER JOIN T03MEG_Element_und_Preisliste ON (MEG_Mengenermittlung.Kostenbereich = T03MEG_Element_und_Preisliste.Kostenbereich) AND (MEG_Mengenermittlung.Element = T03MEG_Element_und_Preisliste.Element), Projektrahmendaten INNER JOIN T01DEF_Regionalfaktoren ON Projektrahmendaten.AGS = T01DEF_Regionalfaktoren.AGS
WHERE (((T01DEF_Untersuchungsjahre.Jahr) Between [Beginn_Betrachtungszeitraum] And [Ende_Betrachtungszeitraum]))
GROUP BY MEG_Mengenermittlung.Teilflaeche_Plangebiet, T01DEF_Untersuchungsjahre.Jahr, MEG_Mengenermittlung.Kostenbereich, T03MEG_Element_und_Preisliste.Element, [Menge]*[Herstellungskosten_einmalig]*[Projektspezifischer_Sonderkostenfaktor]*[Faktor_BKI]
...
excelpfad = os.path.join(ausgabeordner,'31_Infrastrukturkosten.xlsx')
...
methodik_grafik = os.path.join(base_path,"2_Tool","31_Infrastrukturkosten","Erlaeuterungstexte","Methodik_KommunaleInfrastrukturkosten.png")
...
ws6.insert_image('B2', haftung_grafik, {'x_scale': 0.32, 'y_scale': 0.32}) #Korrigiert Verzerrung die bei 1x1 auftritt
