...
class ToolValidator(object):
  ...
  def initializeParameters(self):
    ...
    heading = "01_Quellpotenziale"
    i+=1 ; self.params[i].category = heading
    i+=1 ; self.params[i].category = heading
    i+=1 ; self.params[i].category = heading

    heading = "02_Zielpotenziale"
    i+=1 ; self.params[i].category = heading
    i+=1 ; self.params[i].category = heading
    i+=1 ; self.params[i].category = heading
    heading = "03_Speichern"
    i+=1 ; self.params[i].category = heading
  ...
  def updateParameters(self):
    ...
    list_uebergabepunkte =[]
    self.params[9].value = "Bitte wählen Sie das Projekt aus"
    ...
    if self.params[i].altered and not self.params[i].hasBeenValidated:
        ...
        i+=1 ; self.params[i].filter.list = list_uebergabepunkte
        self.params[9].value = "Bitte wählen Sie den zu bearbeitenden Übergabepunkt aus"
    ...
    if self.params[i].altered and not self.params[i].hasBeenValidated:
        ...
        tablepath_uebergabepunkte = os.path.join(base_path,'3_Projekte',projectname,'FGDB_25_Verkehrserzeugung_'+projectname+'.gdb','L01_Uebergabepunkte')
        sql = "Punkt_Name ='"+self.params[1].value +"'"
        rows_uebergabepunkte = arcpy.SearchCursor(tablepath_uebergabepunkte,sql)
        try:
            for row in rows_uebergabepunkte:
        ...
        self.params[9].value = "Zum Speichern bitte das Kästchen anklicken"

    if self.params[8].value == True and not self.params[8].hasBeenValidated:
        ...
        tablepath_uebergabepunkte = os.path.join(base_path,'3_Projekte',projectname,'FGDB_25_Verkehrserzeugung_'+projectname+'.gdb','L01_Uebergabepunkte')
        Update = arcpy.UpdateCursor(tablepath_uebergabepunkte,sql)
        for row in Update:
            row.QP_Arbeit = self.params[2].value
            row.QP_Versorgung = self.params[3].value
            row.QP_Bildung = self.params[4].value
            row.ZP_Arbeit = self.params[5].value
            row.ZP_Versorgung = self.params[6].value
            row.ZP_Bildung = self.params[7].value
            Update.updateRow(row)
        del Update
        self.params[9].value ="Die Eingaben für den Übergabepunkt " + str(self.params[1].value) + " wurden gespeichert. Wählen Sie den nächsten Übergabepunkt aus oder beenden Sie die Eingabe mit 'OK'"
