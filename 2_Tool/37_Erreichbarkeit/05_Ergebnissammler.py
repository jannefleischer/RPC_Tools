...
import arcpy, os, xlsxwriter, imp, csv, sys, datetime
...
workspace_texte_erlaeterung = os.path.join(base_path,'2_Tool','37_Erreichbarkeit','Erlaeuterungstexte')
workspace_projekt_ergebnisse = os.path.join(base_path,'3_Projekte',projektname, 'Ergebnisausgabe', 'Abbildungen')
workspace_wertveraenderung = os.path.join(base_path,'3_Projekte',projektname,'FGDB_34_Standortkonkurrenz_Supermaerkte_'+projektname+'.gdb')
...
def check_planungsflaeche():
    cur_Search=arcpy.SearchCursor(os.path.join(workspace_projekt,"Flaechenbilanz_Planung_Prozent"))
    typus=0
    for a in cur_Search:
