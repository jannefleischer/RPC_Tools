...
import arcpy, os, inspect, sys, xlsxwriter, urllib, re, time, datetime, imp, csv
...
if arcpy.GetParameterAsText(1) != "":
    orte.append(arcpy.GetParameterAsText(1).replace('(','').replace('-','').replace(')','').encode('utf-8'))
if arcpy.GetParameterAsText(2) != "":
    orte.append(arcpy.GetParameterAsText(2).replace('(','').replace('-','').replace(')','').encode('utf-8'))
if arcpy.GetParameterAsText(3) != "":
    orte.append(arcpy.GetParameterAsText(3).replace('(','').replace('-','').replace(')','').encode('utf-8'))

#Pfade einrichten
base_path = str(sys.path[0]).split("2_Tool")[0]

workspace_projekt_erreichbarkeit = os.path.join(base_path,'3_Projekte',projektname,'FGDB_37_Erreichbarkeit_Projekt_'+projektname+'.gdb')
...
##        arcpy.AddMessage("http://reiseauskunft.bahn.de/bin/query.exe/dn?S="+bahn_start+"&Z="+ziel+"%20Hbf&date="+bahn_wochentag+"&time="+bahn_uhrzeit[0]+"&start=1")
        for zeilen in f:
                try:
...
with open(os.path.join(ausgabeordner,"Zentrale_Orte_temp.csv"), 'wb') as csvfile:
