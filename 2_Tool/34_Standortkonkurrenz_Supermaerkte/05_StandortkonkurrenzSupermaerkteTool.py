...
import arcpy, xlsxwriter, imp, csv,  datetime, math
import math,sys
import gc,os
from decimal import Decimal

def ErgebnisExport(projektname):
    ...
    workspace_basis_daten_wert=os.path.join(base_path,'2_Tool',"34_Standortkonkurrenz_Supermaerkte",'FGDB_34_Standortkonkurrenz_Supermaerkte_Tool.gdb')
    workspace_texte_erlaeterung = os.path.join(base_path,'2_Tool','34_Standortkonkurrenz_Supermaerkte','Erlaeuterungstexte')
    workspace_projekt = os.path.join(base_path,'3_Projekte',projektname,'FGDB_11_Definition_Projekt_'+projektname+'.gdb')
    workspace_wertveraederung = os.path.join(base_path,'3_Projekte',projektname,'FGDB_34_Standortkonkurrenz_Supermaerkte_'+projektname+'.gdb')
    ...
    mxd_template = arcpy.mapping.MapDocument(os.path.join(base_path, "2_Tool","34_Standortkonkurrenz_Supermaerkte","34_Template_Kartenerzeugung","34_KK_Bindung_Template.mxd"))
    ...
    templatepath = os.path.join(base_path,"2_Tool","34_Standortkonkurrenz_Supermaerkte","34_Template_Kartenerzeugung","template.gdb")
    ...
    excelpfad = os.path.join(ausgabeordner,'34_Standortkonkurrenz_Supermaerkte.xlsx')
    ...
    try:
        ...
        arcpy.AddError("Notwendige Datei konnte nicht kopiert werden.")
        exit(1)
...
##    arcpy.AddMessage(Gemeindeklasse)
    if Gemeindeklasse==1:
    #Ueber_100kEW:
    ...
    #Zwischen_10k_20kEW:
        ...
        arcpy.AddError("Fehler bei den Gemeindeklassen")
...
def verkaufsflaeche_zent():
    ...
    try:
        ...
        arcpy.AddError("Verkaufsflaechen Tabelle konnte nicht erstellt werden. Ggf. per Hand loeschen.")
        exit(1)
...
def berechnung_siedlungsdaten(shp):
    ...
    try:
        ...
        arcpy.AddError("Lebensmittel Spalte konnte nicht hinzugefuegt werden - Abbruch")
        exit(1)
...
##            print g
        ...
        arcpy.AddError("Unexpected Error - Kontaktieren sie den Programmierer")
        exit(1)
...
if arcpy.Exists(tablepath_siedlungszellen) and  arcpy.Exists(tablepath_standorte)and  arcpy.Exists(tablepath_neuerMarkt) and  arcpy.Exists(tablepath_zentren) and arcpy.Exists(tablepath_neuer_markt)and arcpy.Exists(tablepath_matrix):
    arcpy.AddMessage("Alle Daten vorhanden und bereit. Tool wird gestartet...")
else:
    arcpy.AddError("Datengrundlage nicht vollstaendig vorhanden. Bitte wiederholen Sie die vorherigen Tools")
    arcpy.AddError("tritt der Error erneut auf kontaktieren Sie uns bitte.")
    exit(1)
...
arcpy.AddMessage("Erstelle Betriebstypenzuteilung")
...
arcpy.AddMessage("Berechnung des Nullfalls starten")
...
arcpy.AddMessage("Die weiteren Berechnungsschritte benoetigen reichlich Zeit")
Kleinste_Distanzen("Distanzmatrix","Standortdaten","Distanzmatrix_dichteste")
arcpy.AddMessage("1/5 Berechnungsschritte")
Attraktivitaet("Standortdaten","Distanzmatrix_dichteste","Distanzmatrix_dichteste_gewi")
arcpy.AddMessage("2/5 Berechnungsschritte")
...
arcpy.AddMessage("3/5 Berechnungsschritte")
schreibe_KK("/Standortdaten","/ET_Kaufkraft","/Statistik")
arcpy.AddMessage("4/5 Berechnungsschritte")
bilde_gesamt("/Uebersicht","/Standortdaten")

arcpy.AddMessage("5/5 Berechnungsschritte")

#------------------------------------------------------------------Berechnung Nullfall beendet
arcpy.AddMessage("Berechnung des Planfalls starten")
...
arcpy.AddMessage("5/5 Berechnungsschritte")
arcpy.AddMessage("Erstelle Abschlussbericht bzw. Deltabetrachtungen")
verlustrechnung()
