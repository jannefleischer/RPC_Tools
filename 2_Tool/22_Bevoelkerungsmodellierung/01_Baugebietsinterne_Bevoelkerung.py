...
# ---------------------------------------------------------------------------
# REGIOPROJEKTCHECK
# 02_Baugebietsinterne_Bevoelkerung.py
#
...
for i in range(betrachtungszeitraum_min,betrachtungszeitraum_max+1): #betrachtungszeitraum
...
FROM T01DEF_Kennwerte RIGHT JOIN T02RECH_Input ON (T01DEF_Kennwerte.Miete_Eigentum = T02RECH_Input.Miete_Eigentum) AND (T01DEF_Kennwerte.Bebauungsart = T02RECH_Input.Bebauungsart) AND (T01DEF_Kennwerte.AlterWE = T02RECH_Input.Alter_WE)
...
#Nullwerte von Einwohner auf 0 setzen

T03ERG_Bevoelkerung_Projekt = os.path.join(workspace_projekt_bevoelkerung,'T03ERG_Bevoelkerung_Projekt')
where_clause = '"Bewohner" IS NULL'
cur = arcpy.UpdateCursor(T03ERG_Bevoelkerung_Projekt, where_clause)
