...
def beschaeftigte_gewerbe(projektname):
    ...
    shape_bkggemeiden=os.path.join(base_path,'1_Basisdaten','FGBD_01_Basisdaten_deutschland.gdb','bkg_gemeinden')
    workspace_Basis_Daten_Versor=os.path.join(base_path,'2_Tool',"34_Standortkonkurrenz_Supermaerkte",'FGDB_34_Standortkonkurrenz_Supermaerkte_Tool.gdb')
    workspace_Basis_Daten=os.path.join(base_path,'2_Tool',"11_Definition_Projekt",'FGDB_11_Definition_Projekt_Tool.gdb')
    ...
    workspace_definition= os.path.join(base_path,'3_Projekte',projektname,'FGDB_11_Definition_Projekt_'+projektname+'.gdb')
...
##  Auskommentiert da andere Tabelle gefunden, die Infos bereits zusammegefasst hat
##        array_branche_anzahl=[]
##        cur_Search_beschaeftigte= arcpy.SearchCursor(os.path.join(workspace_definition,"Gewerbe_Beschaeftigte"))
##
##        for c in cur_Search_beschaeftigte:
##            if len(array_branche_anzahl)==0:
##                array_branche_anzahl.append([str(c.Branche),c.Anzahl])
##            else:
##                if len([t for t in array_branche_anzahl if t[0]==c.Branche])>0:
##                    #bereits vorhanden
##                    for z in array_branche_anzahl:
##                        if z[0]==c.Branche and z[1]<c.Anzahl:
##                            z[1]=c.Anzahl
##
##
##                else:
...
###### Charts anlegen
    ...
    if arcpy.Exists(os.path.join(base_path,'3_Projekte',projektname,'FGDB_11_Definition_Projekt_'+projektname+'.gdb',"Gewerbe_Beschaeftigte_Zahlen")):
        path_beschaeftigte_Zahlen= os.path.join(base_path,'3_Projekte',projektname,'FGDB_11_Definition_Projekt_'+projektname+'.gdb',"Gewerbe_Beschaeftigte_Zahlen")
        datenauslesen=[]
