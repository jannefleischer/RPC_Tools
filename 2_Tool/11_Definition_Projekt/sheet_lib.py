...
def infosheet(projektname, wirkungsfeld, wb):
    import arcpy, os, sys, datetime, xlsxwriter, imp
    ...
    tablepath_Gewerbe = os.path.join(workspace_factsheet,'Gewerbe_Teilflaechen')
    ...
    eingangstabellen = [(workspace_factsheet,'Flaechenbilanz_Planung_Prozent')]
    ausgabetabelle = (workspace_factsheet,'Flaechenbilanz_Planung_gruppiert')

    sql = """SELECT Flaechenbilanz_Planung_Prozent.Teilflaeche_Plangebiet, Flaechenbilanz_Planung_Prozent.Startjahr, Flaechenbilanz_Planung_Prozent.Flaechennutzung_S1, Flaechenbilanz_Planung_Prozent.Flaechennutzung_S2, Sum(Flaechenbilanz_Planung_Prozent.Flaeche_ha) AS Flaeche_ha INTO Flaechenbilanz_Planung_gruppiert
    ...
    for row in rows:
        ...
        try:
            ...
            arcpy.AddMessage("Teilgebietesflaechenbilanz fehlerhaft")
            multi = 0
    ...
    Besch = 0
    for row in rows:
        Besch_teil = row.getValue("beschaeftigte")
