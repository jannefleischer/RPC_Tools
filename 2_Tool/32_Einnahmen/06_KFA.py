...
import arcpy, os, inspect, pyodbc, shutil, gc, sys, datetime, xlsxwriter, imp, urllib2, json
...
projektRahmendaten = os.path.join(base_path ,"3_Projekte",  projektname ,"FGDB_11_Definition_Projekt_" + projektname +".gdb","Projektrahmendaten")
...
projektBevoelkerung = os.path.join(workspace_projekt_bevoelkerung, "T03ERG_Bevoelkerung_Projekt")
...
rows = arcpy.UpdateCursor(KRU_01_Kreisumlagesaetze_Wirkraum)
for row in rows:
    AGS5 = row.AGS5
    if AGS5[-3:] == "000":
        rows.deleteRow(row)
...
sql = """SELECT KFA_03_Jahresdaten.Jahr, KFA_03_Jahresdaten.AGS, KFA_03_Jahresdaten.AGS_VG, KFA_03_Jahresdaten.AGS_Regenesis,  ([Bewohner]*[AWU_Wanderungsfaktor])*-1 AS DeltaEW, KFA_03_Jahresdaten.DeltaGrSt, KFA_03_Jahresdaten.DeltaGewSt, KFA_03_Jahresdaten.DeltaUmsSt, KFA_03_Jahresdaten.DeltaEinkSt, KFA_03_Jahresdaten.DeltaFaLeiA INTO KFA_03_Jahresdaten_temp
FROM (KFA_03_Jahresdaten LEFT JOIN AWU_WanderungsfaktorEW ON KFA_03_Jahresdaten.AGS = AWU_WanderungsfaktorEW.AGS) LEFT JOIN T03ERG_Bevoelkerung_Projekt ON KFA_03_Jahresdaten.Jahr = T03ERG_Bevoelkerung_Projekt.Jahr;
"""
...
#DeltwEinwohner eigene Kommune addieren
...
sql = """SELECT T03ERG_Bevoelkerung_Projekt.Jahr, Sum(T03ERG_Bevoelkerung_Projekt.Bewohner) AS EW INTO KFA_Bevoelkerung_temp
FROM T03ERG_Bevoelkerung_Projekt
GROUP BY T03ERG_Bevoelkerung_Projekt.Jahr;
"""
...
rows = arcpy.SearchCursor(GRST_Grundsteuereinnahmen)

for row in rows:
    GrStEinnahmen = row.GrStEinnahmen
    jahr = row.Jahr
    ...
    rows_update = arcpy.UpdateCursor(KFA_03_Jahresdaten,where_clause)
    for row_update in rows_update:
        row_update.setValue("DeltaGrSt", GrStEinnahmen)
        rows_update.updateRow(row_update)
...
rows = arcpy.SearchCursor(GWST_06_Bilanz)

for row in rows:
    GWST = row.SummevonGWST_EUR
    jahr = row.Jahr
    ags = row.AGS
    ...
    rows_update = arcpy.UpdateCursor(KFA_03_Jahresdaten,where_clause)
    for row_update in rows_update:
        row_update.setValue("DeltaGewSt", GWST)
        rows_update.updateRow(row_update)
...
rows = arcpy.SearchCursor(EKST_06_Bilanz)

for row in rows:
    EKST = row.Bilanz_EST_EUR
    jahr = row.Betrachtungsjahr
    ags = row.AGS
    ...
    rows_update = arcpy.UpdateCursor(KFA_03_Jahresdaten,where_clause)
    for row_update in rows_update:
        row_update.setValue("DeltaEinkSt", EKST)
        rows_update.updateRow(row_update)
...
rows = arcpy.SearchCursor(FLA_01_Ergebnis)
...
for row in rows:
    ...
    rows_update = arcpy.UpdateCursor(KFA_03_Jahresdaten,where_clause)
    for row_update in rows_update:
        row_update.setValue("DeltaFaLeiA", FLA)
        rows_update.updateRow(row_update)
...
cur_in = arcpy.SearchCursor(KFA_03_Steuerkraftmesszahl)

#gemeinde_counter = len(cur_in)

for row in cur_in:
    ags = row.AGS
    jahr = row.Jahr
    EW_Referenz = row.EW_Referenz
    kreisfrei = row.Kreisfrei
    HebGrStA = row.HebGrStA
    HebGrStB = row.HebGrStB
    HebGewSt = row.HebGewSt
    DeltaGrST = row.DeltaGrST
    DeltaGewSt = row.DeltaGewSt
    DeltaUmsSt = row.DeltaUmsSt
    DeltaEinkSt = row.DeltaEinkSt
    DeltaFaLeiA = row.DeltaFaLeiA

    #Zeige Fortschrittsanzeige
    #message = "- noch " + str(gemeinde_counter) + " Berechnungen."
    #arcpy.AddMessage(gemeinde_counter)
    ...
    cur_rahmendaten = arcpy.SearchCursor(KFA_Rahmendaten, where_clause)
    for row_rahmendaten in cur_rahmendaten:
        grundbetrag = row_rahmendaten.Grundbetrag
        ausgleichsfaktor = row_rahmendaten.Ausgleichsfaktor
        Nivellierter_Hebesatz_GrSt_B = row_rahmendaten.Nivellierter_Hebesatz_GrSt_B
        Faktor_Grundsteuer_B = row_rahmendaten.Faktor_Grundsteuer_B
        Nivellierter_Hebesatz_GewSt_netto = row_rahmendaten.Nivellierter_Hebesatz_GewSt_netto
        Faktor_Gewerbesteuer = row_rahmendaten.Faktor_Gewerbesteuer
        Faktor_Umsatzsteuer = row_rahmendaten.Faktor_Umsatzsteuer
        Faktor_Anteil_Einkommensteuer = row_rahmendaten.Faktor_Anteil_Einkommensteuer
        Faktor_Familienleistungsausgleich = row_rahmendaten.Faktor_Familienleistungsausgleich
        Faktor_Zuweisungen_in_Kreisumlage = row_rahmendaten.Faktor_Zuweisungen_in_Kreisumlage
    ...
    cur_update = arcpy.UpdateCursor(KFA_03_Steuerkraftmesszahl,where_clause_update)

    for update_row in cur_update:
        update_row.deltaSKMZ = deltaSKMZ
        update_row.Grundbetrag = grundbetrag
        update_row.Ausgleichsfaktor = ausgleichsfaktor
        update_row.Faktor_Zuweisungen_KU = Faktor_Zuweisungen_in_Kreisumlage

        cur_update.updateRow(update_row)

    del update_row, cur_update
...
cursor_statische_daten = arcpy.SearchCursor(KFA_02_Statische_Daten)

for row_statische_daten in cursor_statische_daten:
    ags = row_statische_daten.AGS
    ags_vg = row_statische_daten.AGS_VG
    ewz = row_statische_daten.EW
    ...
    for row in cursor:
        ew = row.Stuetzwert_EWZ
        vf = row.Stuetzwert_Veredelungsfaktor
        werte = (ew, vf)
...
excelpfad = os.path.join(ausgabeordner,'32_Einnahmen_Kommunaler_Finanzausgleich.xlsx')
...
methodik_grafik = os.path.join(base_path,"2_Tool","32_Einnahmen","Erlaeuterungstexte","Methodik_06_KFA.png")
...
haftung_grafik = os.path.join(base_path,"2_Tool","31_Infrastrukturkosten","Erlaeuterungstexte","Haftungsausschluss.png")
