...
gemeindegroessenklasse64 = arcpy.GetParameterAsText(1)
grundsteuerhebesatz = arcpy.GetParameterAsText(2)

monatsrohmiete_EFH = float(arcpy.GetParameterAsText(3).replace(",","."))
monatsrohmiete_DHH = float(arcpy.GetParameterAsText(4).replace(",","."))
monatsrohmiete_RH = float(arcpy.GetParameterAsText(5).replace(",","."))
monatsrohmieteMFH = float(arcpy.GetParameterAsText(6).replace(",","."))
monatsrohmieteMFH_G = float(arcpy.GetParameterAsText(6).replace(",","."))

jahresrohmiete_garage = float(arcpy.GetParameterAsText(7).replace(",","."))
jahresrohmiete_carport = float(arcpy.GetParameterAsText(8).replace(",","."))
...
rows1 = arcpy.SearchCursor(einheitswert)
for row1 in rows1:
    einheitswert_value = row1.Einheitswert_Summe
    gebauedetyp = row1.Gebaeudetyp
    ...
    rows2 = arcpy.UpdateCursor(einheitswert, where)
    for row2 in rows2:
        row2.setValue("Einheitswert_bis38346_EUR",Steuermesszahl_bis38346_EUR)
        row2.setValue("Einheitswert_ueber38346_EUR",Steuermesszahl_ueber38346_EUR)
...
excelpfad = os.path.join(ausgabeordner,'32_Einnahmen_Grundsteuer.xlsx')
...
methodik_grafik = os.path.join(base_path,"2_Tool","32_Einnahmen","Erlaeuterungstexte","Methodik_04_Grundsteuer.png")
