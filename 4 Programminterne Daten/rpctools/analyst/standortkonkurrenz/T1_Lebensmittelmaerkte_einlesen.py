    ...
    def run(self):
    	...
    	def xlsladen_undStandorteerstellen(pfad):
    		try:
    			...
    			messages.AddMessage("Neue Standorte -> Alte Tabelle geleert")
    		...
    		##                    counter =39
    				...
    				try:
    					...
    					messages.AddErrorMessage("Dateneingabe unvollständig oder falsches Format - Verkaufsflaeche fehlt")
    					exit(1)
    					pass
    				...
    				try:
    					#messages.AddMessage(float(worksheet.cell_value(curr_row, curr_cell)))
    				...
    				if     Betriebstyp=="" or   Strasse=="" or   Ort=="" or  Postleitzahl=="" or  VKFL=="":
    					...
    					messages.AddErrorMessage("Ueberprufen sie Zeile "+ str(counter))
    					exit(1)
    	...
    	def georeferenz():
    		counter=0
    		cur_Standorte_temp=arcpy.UpdateCursor(join(workspace_projekt,"Standorte_temp"))
    		messages.AddMessage("Georeferenzieren der Feature")
    		for zeile in cur_Standorte_temp:
    			...
    			if zeile.Lat ==None or zeile.Lon ==None or zeile.Lat==0 or zeile.Lon==0:
    				...
    				counter=zeile.Id
    				if (counter%50)==0:
    					messages.AddMessage(str(counter)+" Features georeferenziert.")
    			...
    			##                    messages.AddMessage(Strasse+" "+ Ort+ " "+ postcode+ " "+house)
    			...
    			##                    messages.AddMessage( url)
    				...
    				if lat==0 and longi==0 and a==0:
    					messages.AddErrorMessage( url)
    					messages.AddErrorMessage("\n\n\n\nFehler: ")
    					messages.AddErrorMessage("Georeferenzierung derzeit nicht moeglich - Georeferenzierungswebsite down bzw. mit Verbindungsproblemen.")
    					messages.AddErrorMessage("Bitte versuchen sie es spaeter erneut.\n")
    			...
    			##                    messages.AddMessage( str(lat)+" "+ str(longi)+ " "+a)
    				...
    				if int(a)<84:
    					...
    					messages.AddErrorMessage( "Diese Georeferenzierung wird dann verwendet.")
    	...
    	def projektzieren():
    		input_features = join( workspace_projekt,"Standorte_temp")
    		output_features_class = join(workspace_projekt,"Standortdaten")
    		...
    		arcpy.AlterAliasName(join(workspace_projekt,"Standortdaten"), 'Standortdaten')

    	##    try:
    	##        if arcpy.Exists("Standorte_temp"):
    	##            arcpy.Delete_management("Standorte_temp")
    	##    except:
    	##        messages.AddMessage("Fehler beim Loeschen des Tempfiles ")
    	def punktlayerzumprojekt():
    	#   Punktlayer zum Porjekt kopieren - vorher alten punktlayer löschen im projekt und neuen hinzufügen
    		try:
    			arcpy.Delete_management(join(workspace_projekt,"Punktlayer_Bev"))
    		except:
    			print "nicht vorhanden"
    			pass

    		try:
    			arcpy.Project_management(join(pfadPunktlayer), join(workspace_projekt,"Punktlayer_Bev"), "PROJCS['WGS_1984_UTM_Zone_32N',GEOGCS['GCS_WGS_1984',DATUM['D_WGS_1984',SPHEROID['WGS_1984',6378137.0,298.257223563]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433]],PROJECTION['Transverse_Mercator'],PARAMETER['False_Easting',500000.0],PARAMETER['False_Northing',0.0],PARAMETER['Central_Meridian',9.0],PARAMETER['Scale_Factor',0.9996],PARAMETER['Latitude_Of_Origin',0.0],UNIT['Meter',1.0]]", "", "GEOGCS['GCS_WGS_1984',DATUM['D_WGS_1984',SPHEROID['WGS_1984',6378137.0,298.257223563]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433],METADATA['World',-180.0,-90.0,180.0,90.0,0.0,0.0174532925199433,0.0,1262]]")
    	##        arcpy.CopyFeatures_management(join(pfadPunktlayer),join(workspace_projekt,"Punktlayer_Bev"))
