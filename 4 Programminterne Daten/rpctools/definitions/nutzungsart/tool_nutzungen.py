    ...
    def run(self):
        ...
        for row in cursor:
            if row[0] == teilflaeche:
                row[1] = beginn_aufsiedlung
                row[2] = dauer_aufsiedlung
                cursor.updateRow(row)

        for i in range(0,int(dauer_aufsiedlung)):

            jahr = int(beginn_aufsiedlung) + i
            ...
            if int(we_efh) >0:
                ...
                row[2] = int(we_efh) / int(dauer_aufsiedlung)
                row[3] = jahr
            ...
            if int(we_zfh) >0:
                ...
                row[2] =  int(we_zfh) / int(dauer_aufsiedlung)
                row[3] = jahr
                Insert.insertRow(row)
            ...
            if int(we_rh) >0:
                row[0] = teilflaeche
                row[1] = "Reihenhaus"
                row[2] =  int(we_rh) / int(dauer_aufsiedlung)
                row[3] = jahr
                Insert.insertRow(row)
            ...
            if int(we_mfh) >0:
                ...
                row[2] =  int(we_mfh) / int(dauer_aufsiedlung)
                row[3] = jahr
                Insert.insertRow(row)
            ...
            #Ein- und Zweifamilienhäuser
            eigentum = int(ant_eigentum_efh)
            miete = 100 - int(ant_eigentum_efh)
            ...
            for nutzungstyp in nutzungstypen:
                ...
                row[1] = teilflaeche
                row[2] = nutzungstyp[1]
                row[3] = float((ew_je_we_efh.split(" ")[0]).replace(",", "."))
                row[4] = jahr
                row[5] = int(we_efh) / int(dauer_aufsiedlung) * nutzungstyp[0] / 100
                row[6] = ant_eigentum_efh
                Insert.insertRow(row)

            #Doppelhäuser
            eigentum = int(ant_eigentum_zfh)
            miete = 100 - int(ant_eigentum_zfh)
            nutzungstypen = [(eigentum,'Eigentum'),(miete,'Miete')]
            ...
            for nutzungstyp in nutzungstypen:
                ...
                row[4] = jahr
                row[5] = int(we_efh) / int(dauer_aufsiedlung) * nutzungstyp[0] / 100
                row[6] = ant_eigentum_zfh
                Insert.insertRow(row)

            #Reihenhäuser
            eigentum = int(ant_eigentum_rh)
            miete = 100 - int(ant_eigentum_rh)
            nutzungstypen = [(eigentum,'Eigentum'),(miete,'Miete')]
            ...
            for nutzungstyp in nutzungstypen:
                ...
                row[4] = jahr
                row[5] = int(we_efh) / int(dauer_aufsiedlung) * nutzungstyp[0] / 100
                row[6] = ant_eigentum_rh
                Insert.insertRow(row)

            #RMehrfamilienhäuser
            eigentum = int(ant_eigentum_mfh)
            miete = 100 - int(ant_eigentum_mfh)
            nutzungstypen = [(eigentum,'Eigentum'),(miete,'Miete')]
        ...
        row = Insert.newRow()
        row.setValue("teilflaeche", teilflaeche)
        row.setValue("branche", gebietstyp)
        row.setValue("Anteil_A", ant_jobs_verarb_gewerbe)
        row.setValue("Anteil_B", ant_jobs_baugewerbe)
        row.setValue("Anteil_C", ant_jobs_handel)
        row.setValue("Anteil_D", ant_jobs_logistik)
        row.setValue("Anteil_E", ant_jobs_freiwisstech)
        row.setValue("Anteil_F", ant_jobs_sonst_dl)
        row.setValue("Anteil_Zuzug", zuzugsquote_gewerbe)
        row.setValue("Anteil_Eigentum", ant_eigentum_gewerbe)
        Insert.insertRow(row)
        del row, Insert
        ...
        def beschaeftigte_gewerbe(projektname):
            ...
            shape_bkggemeiden=self.folders.get_base_table('FGBD_Basisdaten_deutschland.gdb','bkg_gemeinden')
        ...
        startjahr = beginn_aufsiedlung
        aufsiedlungsdauer = dauer_aufsiedlung
        gebtyp = gebietstyp
        zuzugsquote = zuzugsquote_gewerbe
        eigentumsquote = ant_eigentum_gewerbe
        Ant_Betr_VerarbGew = ant_jobs_verarb_gewerbe
        Ant_Betr_BauGew = ant_jobs_baugewerbe
        Ant_Betr_HanKfZ = ant_jobs_handel
        Ant_Betr_VerLag = ant_jobs_logistik
        Ant_Betr_FrWiTeDi = ant_jobs_freiwisstech
        Ant_Betr_SoDi = ant_jobs_sonst_dl
        ...
        row = Insert.newRow()
        row.Teilflaeche_Plangebiet = teilflaeche
        row.Verkaufsflaeche = verkaufsflaeche
        Insert.insertRow(row)
        del row, Insert
        gc.collect()