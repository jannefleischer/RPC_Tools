...
def install_packages(python_path, platform):
    ...
    ret = process.wait()
    if ret:
        print('pip nicht richtig installiert')
    else:
        print('pip installiert')

    ##Installing packages
    import pip
    print('wheel_path; {}'.format(wheel_path))

    for package, filename in used_packages.iteritems():
        print('{p}: {f}'.format(p=package, f=filename))
        try:
            del numpy, matplotlib
        except:
            pass
        ...
        ret = process.wait()
        if ret:
            print("Paket " + package + " konnte ggf. nicht installiert werden." + "\n")
        try:
    ...
    if ret:
        print('rpctools konnte nicht installiert werden')
    else:
        print("RPCTools installiert")
...
if __name__ == '__main__':
    ...
    python_dirs = [
                   ['C:\\Python27\\ArcGISx6410.4','win_amd64'],
                   ['C:\\Python27\\ArcGIS10.4', 'win32'],
                   ['C:\\Python34','win_amd64']]

    for directory in python_dirs:
        if os.path.exists(directory[0]):
            install_packages(directory[0], directory[1])
