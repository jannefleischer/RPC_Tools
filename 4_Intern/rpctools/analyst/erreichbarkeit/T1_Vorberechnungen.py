...
def main(parameters, messages):
    ...
    try:
    ##    messages.AddMessage(arcpy.Exists(os.path.join(workspace_projekt,"zwischenspeicher_sort1")))
    ...
    mxd_template = arcpy.mapping.MapDocument(os.path.join(base_path, "2_Tool","D_Erreichbarkeit","Template_Kartenerzeugung","Haltestellenkarte_Template.mxd"))
    mxdpfad = os.path.join(workspace_projekt_ergebnisse,'Karte_Haltestellen.mxd')
    mxd_template.saveACopy(mxdpfad)
    
    # Ersetze Datenquelle
    Haltestellen_mxd = arcpy.mapping.MapDocument(mxdpfad)
    templatepath = os.path.join(base_path,"2_Tool","D_Erreichbarkeit","Template_Kartenerzeugung","template.gdb")
    resultpath = os.path.join(base_path,"3_Projekte",projektname,"FGDB_Erreichbarkeit_Projekt_"+projektname+".gdb")
    Haltestellen_mxd.findAndReplaceWorkspacePaths(templatepath, resultpath)
    
    templatepath = os.path.join(base_path,"3_Projekte", "Template","FGDB_Erreichbarkeit_Projekt_Template"+".gdb")
    resultpath = os.path.join(base_path,"3_Projekte",projektname,"FGDB_Definition_Projekt_"+projektname+".gdb")
    Haltestellen_mxd.findAndReplaceWorkspacePaths(templatepath, resultpath)
    ...
    #Exportiere Ergebnis
    arcpy.mapping.ExportToJPEG(Haltestellen_mxd, os.path.join(workspace_projekt_ergebnisse, "Karte_Haltestellen.jpg"), "PAGE_LAYOUT",resolution=150)
